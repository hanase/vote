...
plot.vote.stv <- function(x, xlab = "Count", ylab = "Preferences", point.size = 2, xmin = 0, ...) {
    ...
    # if xmin is larger than 0, remove data prior that and change some settings for Quota annotation
    if(xmin > 0){
        dfl <- dfl[Count >= xmin]
        dfq <- dfq[Count >= xmin]
        dfe <- dfe[Count >= xmin]
